 ...
 */
router.post('/Upload', multer(multerConfig).any('uploadFiles'), function (req, res) {
    replaceRequestParams(req, res);
...
router.post('/', function (req, res) {
    ...
    if (req.body.action == "read") {
        (async () => {
            ...
            // Declare rootName explicitly before using it
            const rootName = (req.body.path == "/") ? path.basename(contentRootPath + req.body.path) : path.basename(contentRootPath + req.body.path);

            cwdFiles.name = rootName;
